...
namespace Gifter.Repositories
{
    public class PostRepository : BaseRepository, IPostRepository
    {
                    ...
                    while (reader.Read())
                    {
                        if (post == null)
                        {
                            post = new Post()
                            {
                                ...
                                //Can instantiate Comments below in the Post Model too as:
        ...
        public List<Post> Search(string criterion, bool sortDesc)
        {
            using (var conn = Connection)
            {
                conn.Open();
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    //the % on either side is considered a Wildcard, so that the criterion search term will retreive the title/caption wether there is text to the left or the right of that search term.
                    var reader = cmd.ExecuteReader();
                ...
                using (var cmd = conn.CreateCommand())
                {
                    var sql = @"
                        SELECT p.Id AS PostId, p.Title, p.Caption, p.DateCreated AS PostDateCreated, 
                          ...
                          FROM Post p 
                     LEFT JOIN UserProfile up ON p.UserProfileId = up.id
                         WHERE p.DateCreated >= @Since DESC";
